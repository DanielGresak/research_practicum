...
def query_weather_data(url, **kwargs):
    ...
    if "cnt" in kwargs:
        ...
        if isinstance(cnt, int):
            ...
            req_url = "{0}lat={1}&lon={2}&cnt={3}&units={4}&appid={5}".format(url, 
                                                                            LATITUDE, 
                                                                            LONGITUDE, 
                                                                            cnt, 
                                                                            UNITS, 
...
def update_weather_forecast():
    ...
    def delete_outdated_forecast():
        ...
        try:
            ...
            # Note: A Unix timestamp is the number, represented as a integer, of seconds between a 
...
def update_current_weather():
    ...
    if current_weather_json is not None:     
        ...
        # it means that these weather phenomena are just not happened for the time of measurement for 
        # the city or location chosen. 
