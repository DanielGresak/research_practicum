...
$(".vertical-menu a").click(function() {
  if ($(this).hasClass("active")) { // If the clicked item is highlighted
    ...
    $(".appbar").animate({
      width: ['toggle']
    }); // Collapse or uncollapse the appbar
    ...
    if (status == "OK") {
      ...
      $(".selectRoute").mousedown(function() {
        ...
        for (let stroke = 0; stroke < directionRenderers.length; stroke++) {
          directionRenderers[stroke].setOptions({
            map: null,
            directions: result,
            routeIndex: stroke
          });
...
function displayTheForecastTime(theRouteId, route, walkingTime, resultTime, startStop, destStop) {
  return new Promise(function(resolve, reject) {
    setTimeout(function() {
      resolve($("#" + theRouteId).append("<span class ='keyValue'>" + getForecastTravelTime(route, walkingTime, resultTime, startStop, destStop) + " minutes</span>"))
    }, 0);
  })
...
$("#login-button").click(function() {
  ...
  if (!emailField.checkValidity()) {
    ...
    $("#login-email-validation-text").css({
      'color': 'red'
    });
...
$("#register-button").click(function() {
  ...
  if (!emailField.checkValidity()) {
    ...
    $("#register-email-validation-text").css({
      'color': 'red'
    });
